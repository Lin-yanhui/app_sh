...
const LURL = "https://f.kyee.com.cn/RightsSystem/LoginHandler.jspx";
const WURL = "https://f.kyee.com.cn/HR/WorkManage/WorkDataManage/WorkDataManageHandler.jspx";

const LJQ_pwd_data = "xxxx";

const LJQ_COOKIE_users = "users=%7B%22os_username%22%3A%2211725%22%2C%22orgId%22%3A210001%7D; "
const LJQ_COOKIE_sys = "currentSystem=8001; "

const username = "11725";
const passwordRsa = "xxxxxx";
...
var firstKey = "KYEE",secondKey = "HRP",thirdKey = "!@#$";
...
var loginUserCheck = function() {
	var options = { method: 'GET',
  url: LURL,
  qs:
   { op: 'loginUserCheck',
     u_name: username,
     firstLoadData: LJQ_pwd_data },
  headers:
   { 'Cache-Control': 'no-cache' } };

request(options, function (error, response, body) {
	if (!error && response.statusCode == 200) {
		console.log('loginUserCheck success body => ' + body);
		resetCookie(response);
		var data = JSON.parse(body);
		if (data.data && data.data.USER_ID && data.data.USER_PASS) {
			orgId = data.data.ORG_ID;
			authKey = data.data.AUTH_KEY;
			timeRandom = data.data.COMMENTS.replace(/:/g, "/");
			login();
		}
	}else {
...
var resetCookie = function(response){
	var setCookie = response.headers['set-cookie'];
	console.log("resetCookie response.headers['set-cookie'] => "+JSON.stringify(setCookie));
	for(var i=0;i<setCookie.length;i++){
		if(!!setCookie[i]){
			var cookieval = setCookie[i].split(";")[0];
			if(cookieval.indexOf("JSESSIONID") > -1){
				jsessionid = cookieval;
				console.log('resetCookie jsessionid => '+jsessionid);
			}
			if(cookieval.indexOf("SERVERID") > -1){
				serverid = cookieval;
				console.log('resetCookie serverid => '+serverid);
			}
		}
	}
}
...
function login() {
	var plainparam = username + "_$$@@_" + passwordRsa + "_$$@@_" + authKey + "_$$@@_"+ timeRandom;
	var secondLoadData = Des_Encrypt(plainparam);
	console.log('Des_Encrypt plainparam => ' + plainparam);
	console.log('Des_Encrypt secondLoadData => ' + secondLoadData);
	console.log('login header cookie => '+LJQ_COOKIE_users + jsessionid + "; " + serverid);
	doLogin(secondLoadData, LJQ_COOKIE_users + jsessionid + "; " + serverid);
}
...
function getdata() {
  ...
  qs:
   { moduleNo: '',
     op: 'dataCollectionQuery1',
     DEPT_GLOBAL_NO: ':-100:20370362:52556621:368768166',
     BEGINDATE: begindate,
     ENDDATE: enddate,
     ORG_ID: '210001',
     page: '1',
  ...
  headers:
   { 'Cache-Control': 'no-cache',
     cookie: LJQ_COOKIE_users + jsessionid + "; " + serverid } };

request(options, function (error, response, body) {
  if (error) throw new Error(error);

  console.log("getdata:" + body);
  mainResponse.send(body);
});
}

var doLogin = function(loadData, cookieHeader){
	var options = { method: 'POST',
	  url: LURL,
	  qs:
	   { op: 'login',
		 secondLoadData: loadData },
	  headers:
	   { 'Cache-Control': 'no-cache',
		 cookie: cookieHeader } };
...
var formatDate = function(offset)
{
	...
	var year = fdate.getFullYear();       //年
	var month = fdate.getMonth() + 1;     //月
	var day = fdate.getDate();            //日
